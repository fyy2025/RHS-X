...
def main():
    ...
    parser.add_argument("--pb_steps", type=int, default=20, help="PB exploration steps per theta")
    ...
    for iter in range(10 * (epoch - 1) + 1, 10 * epoch + 1):
        ...
        MCMC.run_mcmc_streaming_rand_start(
            ...
            out_jsonl=f"./output_all/mcmc_samples_epoch{epoch}_iter{iter}.jsonl",
            progress_json=f"./output_all/mcmc_progress_epoch{epoch}_iter{iter}.json",
        )
        mcmc_res = MCMC.load_mcmc_res_from_jsonl(
            f"./output_all/mcmc_samples_epoch{epoch}_iter{iter}.jsonl")
    ...
    with open(f"./output_all/sim1_all_result{epoch}.pkl", "wb") as f:
        pickle.dump(overall_result, f)
    print(f"Saved epoch {epoch} to ./output_all/sim1_all_result{epoch}.pkl")
