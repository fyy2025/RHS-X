...
# ---------------------------------------------------------------------------
# Scenario 2 — AIS seeded from RANDOM starts (companion to
# simulation_scenario2_final.py, which seeds AIS from the RPS).
#
# Everything is held identical to the RPS-seeded script — data generation,
# seeds, problem structure, score_s (g-prior), annealing ladder, AISConfig,
# eps1/eps2 — so the AIS_{theta} outputs are directly comparable.  The ONLY
# difference is the seed set fed into the annealing pipeline:
#
#     RPS-seeded   :  init_states = RPS_states               (near-optimal)
#     random-seeded:  init_states = uniformly-random states  (this script)
#
# The number of random seeds is matched to len(RPS_states) per theta, so the
# comparison isolates seed *quality* (RPS vs random) at equal seed *count*.
# MCMC and PAC-Bayes are dropped (unchanged from the original run); RPS is
# still computed as the reference set and to fix the seed count.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Scenario 2 — AIS from RANDOM starts (RPS-seeded comparison baseline)")
    ...
    print(f"Epoch {epoch} | eps1={eps1} eps2={eps2} | "
          f"thetas={thetas} | AIS seed=RANDOM")

    os.makedirs("./output2_randstart", exist_ok=True)

    # ── AIS hyper-parameters (identical to the RPS-seeded script) ────────────
    N_ITER           = 50000   # kept for parity of constants; MCMC not run here
    N_BURN           = 20000
    N_THIN           = 10
    n_paths          = 300
    n_levels         = 20
    ...
    for iter in range(5 * (epoch - 1) + 1, 5 * epoch + 1):
        ...
        # ── Per-theta: RPS reference + AIS seeded from RANDOM states ─────────
        for theta in thetas:
            H = np.inf
            ...
            # -- RPS (kept as reference set / seed-count source) --
            ...
            # -- Random seed set, matched in size to the RPS --
            ...
            # -- AIS seeded from the random states (same pipeline as RPS run) --
            start   = time.time()
            ...
            result[f"AIS_{theta:g}_seed_type"] = "random"
            result[f"AIS_{theta:g}_n_seeds"]   = len(rand_states)

        overall_result.append(result)

    out_path = f"./output2_randstart/sim2_randstart_result{epoch}.pkl"
