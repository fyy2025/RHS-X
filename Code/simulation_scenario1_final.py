...
def main():
    ...
    parser.add_argument("--theta_init",   type=float, default=7.8,
    ...
    for iter in range(5 * (epoch - 1) + 1, 5 * epoch + 1):
        ...
        # ── Exact enumeration ───────────────────────────────────────────────
        start = time.time()
        all_partitions, losses_exact = AIS.enumerate_all_states_and_losses(
        ...
        true_log_post  = [-i[1] for i in losses_exact]
        ...
        result["rps_init_n_states"] = len(rps_init)
        ...
        # ── Per-epsilon: RPS / AIS+RPS / PAC-Bayes ─────────────────────────
        for eps in epsilons:
            theta = map_q * (1 + eps)
            H = np.inf
