...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare AIS-RPS vs AIS-SSS — same seeds as simulation_scenario1.py")
    parser.add_argument('--epoch',   type=int,   required=True, help="Epoch number")
    parser.add_argument('--n_steps', type=int,   default=200,   help="SSS steps per theta")
    ...
    eps1         = args.eps1
    eps2         = args.eps2
    print(f"Epoch {epoch} | SSS n_steps={n_steps} | AIS eps1={eps1} eps2={eps2}")

    # AIS config — match simulation_scenario1.py
    n_paths         = 300
    n_levels        = 20
    moves_per_level = 5
    ...
    # ------------------------------------------------------------------ #
    # Problem setup — identical to simulation_scenario1.py
    # ------------------------------------------------------------------ #
    lamb = 1
    M    = 2
    R    = np.array([4, 3])
    ...
    sigma_00 = None
    mu_00    = np.array([0]);      var_00 = np.array([1])
    sigma_01 = np.array([[1]])
    mu_01    = np.array([-1]);     var_01 = np.array([1])
    sigma_10 = np.array([[1, 0]])
    mu_10    = np.array([-2, -3]); var_10 = np.array([1, 1])
    ...
    for iter in range(10*(epoch-1)+1, 10*epoch+1):
        np.random.seed(iter * 42)   # identical seed to simulation_scenario1.py
        ...
        # ---------------------------------------------------------------- #
        # Per-theta: RPS → SSS → AIS-RPS → AIS-SSS
        # ---------------------------------------------------------------- #
        cfg = AIS.AISConfig(
        ...
        for theta in [7.8, 8, 8.2, 8.4, 8.6]:
            ...
            result[f"RPS_{theta}_coverage"]  = MCMC.posterior_mass_coverage(log_alpha, true_log_post)
    ...
    with open(f"./output_sss/sim1_sss_result{epoch}.pkl", "wb") as f:
        pickle.dump(overall_result, f)
    print(f"Saved epoch {epoch} to ./output_sss/sim1_sss_result{epoch}.pkl")
